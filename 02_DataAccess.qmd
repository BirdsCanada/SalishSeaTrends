...
PSSS_loaded <- read.csv("Data/PSSS Survey Data - 13 Mar 2025.csv") #copy contains wyear 2023
...
# Call the function passing the loaded data frame
psss_to_bmde(PSSS_loaded)
...
Read in the clean datasets and combine.

```{r}

in.BCCWS<-read.csv("Data/in.BCCWS.csv")
in.PSSS<-read.csv("Data/in.PSSS.csv")

event.BCCWS<-read.csv("Data/event.BCCWS.csv")
event.PSSS<-read.csv("Data/event.PSSS.csv")
...
Not all species will be included in an analysis. Some species are detected too infrequently to be included or they may not be species of focus for your study area. In the next section, you will be given the option to manually [select focal species or guilds](#3.1.2Analysis) for the analysis. 

Here we filter for species that are detected by both surveys. Generally, PSSS monitors just targets, whereas the BCCWS monitors more non-targets. In the analysis script we will set some additional [minimum data requirements](#3.1.3Analysis) to remove species that do not have enough data to estimate trends.
