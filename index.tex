...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#Determine which SpeciesCode each ProjectCode share in the full dataset}
\NormalTok{sp.bccws}\OtherTok{\textless{}{-}}\NormalTok{in.data }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{filter}\NormalTok{(ProjectCode }\SpecialCharTok{==} \StringTok{"BCCWS"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(CommonName) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{distinct}\NormalTok{()}
\NormalTok{sp.psss}\OtherTok{\textless{}{-}}\NormalTok{in.data }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{filter}\NormalTok{(ProjectCode }\SpecialCharTok{==} \StringTok{"PSSS"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(CommonName) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{distinct}\NormalTok{()}

\NormalTok{common.species}\OtherTok{\textless{}{-}}\FunctionTok{intersect}\NormalTok{(sp.bccws}\SpecialCharTok{$}\NormalTok{CommonName, sp.psss}\SpecialCharTok{$}\NormalTok{CommonName) }
\CommentTok{\#There were 53 species in common between BCCWS and PSSS that are carried forward for the analysis}

\CommentTok{\#filter the full dataset to only include the common species}
\NormalTok{sp.data}\OtherTok{\textless{}{-}}\NormalTok{in.data[in.data}\SpecialCharTok{$}\NormalTok{CommonName }\SpecialCharTok{\%in\%}\NormalTok{ common.species,]}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{guild}\OtherTok{\textless{}{-}}\FunctionTok{read.csv}\NormalTok{(}\StringTok{"Data/GuildList.csv"}\NormalTok{)}
\NormalTok{guild}\OtherTok{\textless{}{-}}\NormalTok{guild[guild}\SpecialCharTok{$}\NormalTok{english\_name }\SpecialCharTok{\%in\%}\NormalTok{ common.species,]}
...
\NormalTok{guild}\OtherTok{\textless{}{-}}\FunctionTok{left\_join}\NormalTok{(guild, family, }\AttributeTok{by=}\FunctionTok{c}\NormalTok{(}\StringTok{"species\_id"}\NormalTok{))}

\NormalTok{sp.data}\OtherTok{\textless{}{-}}\FunctionTok{left\_join}\NormalTok{(sp.data, guild, }\AttributeTok{by=}\FunctionTok{c}\NormalTok{(}\StringTok{"CommonName"} \OtherTok{=} \StringTok{"english\_name"}\NormalTok{))}
