...
for(i in 1:length(sp.list)){
  ...
  if(nrow(dat)>0){        
    ...
    if(min.data==TRUE){
     ...
     # Suppose 1000 posterior samples in post.sample1
     tmp0 <- bind_rows(
     ...
     tmp1 <- tmp0 %>%
      ...
      slope_summary <- tau_trends %>%
        ...
        summarise(
          ...
          # If you included total_percent_change above, add:
      ...
      years_span <- length(unique(years)) - 1  # number of steps between min/max year
